...
export function renderFavoriteStationList(parent: HTMLElement, options: FavoriteStationListOptions): void {
  ...
  const isPlaying = options.plugin.getIsPlaying();
  const currentStationId = options.plugin.getCurrentStation()?.stationuuid;
  const activeControlColor = options.plugin.settings.pomodoroTimerColor;
  const canSelectStation = Boolean(options.onSelectStation);
  const hasPlaybackActions = Boolean(options.onPlayStation || options.onStopStation);
  ...
  favorites.forEach((station, index) => {
    ...
    options.plugin.createStationLogo(row, station, STATION_LOGO_OPTIONS);
    ...
    const hasActions = hasPlaybackActions || Boolean(options.onRemoveStation);
    const actions = hasActions ? row.createDiv({ cls: 'streamradio-row-actions' }) : null;
    const isActiveStationPlaying = isPlaying && currentStationId === station.stationuuid;

    if (actions && hasPlaybackActions) {
      ...
      playButton.style.setProperty('--streamradio-active-control-color', activeControlColor);
